...
- 446字节（参数+引导程序）
- 64字节磁盘分区表DiskPartitionTable，每个分区描述符16B，共有4个**主分区**
- 0x55AA

&nbsp;

最多一个扩展分区，可含ide有63个、scsi有15个分区。
...
子扩展分区结构和硬盘一样，第一个扇区类似于MBR

扩展引导扇区EBR的64字节DPT中，

第一个分区表项：分区数据，该逻辑分区最开始的扇区（操作系统引导扇区，OBR，内核加载器，DosBR）

第二个分区表项：后边分区的指针，即下一个子扩展分区的EBR扇区
