...
管道也被视为文件，只是该文件并不存在于文件系统上，而是只存在于内存中。 

管道其实就是内核空间中的内存缓冲区。 

管道使用环形缓冲区，

当缓冲区已满时，生产者要睡眠，并在睡眠前唤醒消费者，

当缓冲区为空时，消费者要睡眠，并在睡眠前唤醒生产者。

当缓冲区满或空时，使一方休眠，这是保证数据不丢失的方法。管道其实就是典型的生产者和消费者问题。

**进程创建管道**，实际上是内核为其返回了用于读取管道缓冲区的文件描述符，一个描述符用于读，另一个描述符用于写。

创建管道之后，调用fork产生子进程，由于是共享管道文件（以及文件读写指针！），以此实现父子进程的通信。

一般情况下，父子进程中都是一个读数据，一个写数据，并不会存在一方又读又写的情况，

因此在父子进程中会分别关掉不使用的管道描述符。

**匿名管道**只能局限用于父子进程间的通信。`int pipe(int pipefd[2])`成功返回 0

mkfifo 来创建**命名管道 **，所有进程都可使用。

&nbsp;

Linux的管道大小为一个页，4096字节。

复用之前定义的**文件结构**的成员：

文件结构对应一个普通文件或目录的 inode，

现在多了管道这种新的文件类型，而管道不需要 inode，

那如何在文件结构中识别管道，而不是误把它当成一般的 inode 来处理呢？

fd_flags 成员：如果此文件结构对应的是管道，那么 fd_flags 的值将是 0xFFFF，不再是 O_RDONLY、 O_WRONLY 等值。

fd_inode 成员：指向管道的内存缓冲区。

fd_pos 成员：管道的打开数。

&nbsp;&nbsp;

我们通过让不同进程PCB里面的文件描述符数组 fd_table 里的元素，指向同一个全局文件结构数组 file_table 的元素，实现进程间通信。这个被共享的 file_table 中的元素，它管理的空间就是管道的缓冲区。

&nbsp;

理论上，管道两端的进程是并行的，但是为了简单，我们只实现了从左往右逐个执行，导致的问题：

当管道的环形缓冲区满了后，生产者进程休眠，只要生产者未退出，后续的消费者进程就没机会执行，因此，生产者永远休眠没机会被唤醒。

对于消费者进程也是，如果管道空了，消费者也会永远阻塞，但此时生产者已经退出了，消费者进程没有机会被唤醒。 

折中方案：管道能传递的最大数据量，是环形缓冲区的大小减一。这样就都不会睡眠了。。。
