...
struct arena {
	...
	/* large为ture时,cnt表示的是arena下边的页框数。（此时申请的空间>1024）详见sys_malloc函数
	 * large为ture时,cnt表示空闲mem_block数量 */
	uint32_t cnt;
	bool large;
};
	...
	* *********************************************************/
	...
	if (*pde & 0x00000001) {			// 页目录项 存在
		...
		*pde = (pde_phyaddr | PG_US_U | PG_RW_W | PG_P_1);			// 注意这行是pde_phyaddr，不是page_phyaddr!!!
		...
		/* 分配到的物理页地址 pde_phyaddr 对应的物理内存页清0，避免里面的陈旧数据引起混乱.
...
/* 在堆中申请size字节内存
 * 1.注意其中对arena结构体的初始化的两种情况。
 * 2.内核进程，从内核内存池 struct pool kernel_pool 里面分配
 *   用户进程，从用户内存池 struct pool user_pool   里面分配【所有用户共用同一个user_pool！】
...
static void mem_pool_init(uint32_t all_mem) {
	...
	/*********    内核内存池和用户内存池位图   ***********
	*   用数组来描述的位图.bits是全局的数据，长度不固定。全局或静态的数组需要在编译时知道其长度。
